...
> 快速查找组件和模块文件位置  
> 最后更新：2025-11-27  
> ⚠️ 思维链 LLM 辅助识别功能暂未可用（开关已锁定，等待修复），相关接口暂不调用  
> 聊天页面与消息组件、渲染设置已完成解耦，UI 调整可集中改动展示层
...
```
emotichat/
├── config.yaml                 # 运行时配置（监听/白名单/路径/日志）
├── config/                     # 配置解析代码（config/runtime.ts）
├── app/                        # Next.js 应用目录
│   ├── (main)/                 # 主应用路由组
│   │   ├── chat/               # 聊天页面
│   │   ├── characters/         # 角色管理页面
...
| `app/(main)/layout.tsx` | 主应用布局（侧边栏、Header 等） |
...
| 文件 | 功能 |
|------|------|
| `chat-screen.tsx` | 聊天页容器（UI 编排） |
| `message-list.tsx` | 消息列表容器 |
| `message-bubble.tsx` | 消息气泡 |
| `message-meta.tsx` | 消息元信息/操作区 |
| `message-streaming.tsx` | 流式渲染与计时器 |
| `message-editor.tsx` | 消息编辑器 |
| `message-actions.tsx` | 消息操作区 |
...
| `thinking-settings-card.tsx` | 思维链折叠与标签设置 |
| `special-fields-card.tsx` | 特殊字段渲染规则设置 |
| `html-rendering-card.tsx` | HTML 渲染开关卡片 |
...
| `macros.ts` | 宏展开 |
| `role-adapter.ts` | Role 适配器（Gemini/OpenAI/Claude 等） |
| `injection.ts` | 注入处理 |
...
| 文件 | 功能 |
|------|------|
| `config.yaml`（根目录） | 运行时配置：监听/白名单/路径/日志，预留账号密码 |
| `config/runtime.ts` | 解析运行时配置、暴露 data/logs 路径与监听选项 |

### 聊天工具 (`lib/chat/`)

| 文件 | 功能 |
|------|------|
| `devmode-stream-interceptor.ts` | DevMode 流拦截器，提取请求体事件 |
...
| 文件 | 功能 |
|------|------|
| `conversation.ts` | 对话状态 Store |
| `uiPreferences.ts` | UI 偏好 Store |
...
| `useRecentConversations.ts` | 最近对话 Hook |
| `useMessages.ts` | 消息 Hook（入口） |
| `useChatController.ts` | 聊天页控制器 Hook（页面数据编排） |
| `useMessageListBehavior.ts` | 消息列表滚动与流式计时 Hook |
| `messages/useMessageTransport.ts` | 消息传输层（AI SDK transport） |
| `messages/useMessageMetadata.ts` | 消息元数据管理 |
| `messages/useThinkingBlockPersistence.ts` | 思维链持久化 |
| `messages/useMessageEditing.ts` | 消息编辑态管理 |
| `useThinkingLLMTester.ts` | 思维链 LLM 测试/模型拉取 |
| `useCharacters.ts` | 角色 Hook |
...
### 按功能查找
**聊天功能**
- 页面容器：`components/chat/chat-screen.tsx` + `hooks/useChatController.ts`
- 发送消息：`components/chat/chat-input.tsx`
- 显示消息：`components/chat/message-list.tsx` + `components/chat/message-bubble.tsx`
- 编辑消息：`components/chat/message-editor.tsx`
- AI 对话 API：`app/api/chat/route.ts`
...
- 变量/宏处理：`lib/prompt/variables.ts` + `lib/prompt/macros.ts`
- Role 适配：`lib/prompt/role-adapter.ts`

**模型配置**
- 模型选择：`components/chat/model-selector.tsx`
- 模型参数：`components/chat/model-parameters.tsx`
- API 密钥：`components/settings/api-keys.tsx`
- 全局端点选择：`components/settings/global-endpoint-selector.tsx`

**配置与存储**
- 运行时配置：根目录 `config.yaml`
- 配置解析：`config/runtime.ts`
- 数据根路径：`data/`（由运行时配置解析）

**调试工具**
- 开发者面板：`components/chat/dev-mode-panel.tsx`
- Token 计数：`components/chat/token-counter.tsx`
- DevMode 流拦截：`lib/chat/devmode-stream-interceptor.ts`

---

## 解耦进度
- 聊天页面：已拆分控制器（`useChatController`）、容器（`chat-screen`）与消息子组件。 
- 消息流：计时/滚动拆分为 `useMessageListBehavior`，流式渲染抽离 `message-streaming`。 
- 渲染设置：拆分为 `thinking-settings-card`、`special-fields-card`、`html-rendering-card`。 
- 配置：环境变量已移除，统一使用根 `config.yaml` + `config/runtime.ts` 解析。
