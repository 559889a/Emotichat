...
- hooks/useMessages.ts：集成聊天状态、网络请求、思维链处理、DevMode 事件，需拆分为请求层、状态层、UI 回调层。
- app/(main)/chat/page.tsx：页面编排与数据加载/事件处理耦合，建议抽离控制器/容器层。
- components/chat/message-bubble.tsx：单文件包含渲染、编辑、计时器、思维链折叠逻辑，建议分离展示与交互逻辑。
- components/chat/message-list.tsx：滚动、流式状态、计时逻辑集中，建议拆出滚动/流状态管理。
- components/settings/rendering-settings.tsx：设置表单与外部调用/测试逻辑耦合，后续可拆为纯表单与异步调用模块。